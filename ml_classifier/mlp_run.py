...
def manual_select(total_list,exclude_list=None):
    ...
    if exclude_list is not None:
        for exclude_label in exclude_list:
            if 'hog' not in exclude_label:
                nouse_list += [f'{exclude_label}_{str(i)}' for i in range(16)]
            else:
                nouse_list += [f'{exclude_label}_{str(i)}' for i in range(256)]
...
def run(train_path,test_path,result_path,output_dir,net_depth=3,exclude_list=None,scale_flag=True,select_flag=False,dim_reduction=False,aug_flag=False,tta=False,**aux_config):
    ...
    test_id = test_df['id'].apply(lambda x: os.path.basename(x))
    ...
    for fold_num,(train_index,val_index) in enumerate(kfold.split(X)):
        ...
        fold_vote_csv['image_id'] = list(test_id)
        ...
        fold_prob_csv['image_id'] = list(test_id)
    ...
    total_vote_csv['image_id'] = list(test_id)
    ...
    total_prob_csv['image_id'] = list(test_id)
...
if __name__ == '__main__':
    feas = ['contrast','dissimilarity','homogeneity','correlation','ASM','energy','hog']
    
    train_path = './dataset/temp_freq/train_temp_freq_features.csv'
    test_path = './dataset/temp_freq/test_temp_freq_features.csv'
    # train_path = '../dataset/hmm_train.csv'
    # test_path = '../dataset/hmm_test.csv'
    result_path = '../analysis/result/Temp_Freq/v6.0-pretrained/submission_ave.csv'
    output_dir = './result/temp_freq'
    ...
    # for i,act in enumerate(ACTIVATION):
        ...
        # exclude_list = ['hog']
        # half
        # exclude_list = random.sample(feas,1)
        exclude_list = ['contrast','dissimilarity','homogeneity','correlation','ASM','energy']
